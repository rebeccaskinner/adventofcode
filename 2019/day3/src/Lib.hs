...
parseDirection s =
  case map Char.toUpper s of
    "L" -> Right L
    "R" -> Right R
    "U" -> Right U
    "D" -> Right D
    s'  -> Left $ "Invalid direction: " <> s'

data MovementVector = MovementVector Direction Int deriving (Eq, Show, Ord)
...
taxicabDistance (Point x y) = (abs x) + (abs y)

type Segment = (Point,Point)

mkRange :: (Ord a, Enum a, Num a) => a -> a -> [a]
mkRange x y
  | x < y = [x..y]
...
points (p1,p2) =
  let xs = mkRange (_pointX p1) (_pointX p2)
      ys = mkRange (_pointY p1) (_pointY p2)
  in [Point x'' y'' | x''  <- xs, y'' <- ys]

addVectorToPoint :: MovementVector -> Point -> Point
addVectorToPoint (MovementVector L n) (Point x y) = Point (x-n) y
addVectorToPoint (MovementVector R n) (Point x y) = Point (x+n) y
addVectorToPoint (MovementVector U n) (Point x y) = Point x (y + n)
addVectorToPoint (MovementVector D n) (Point x y) = Point x (y - n)

parseMovementVector :: String -> Either String MovementVector
parseMovementVector s
  | length s < 2 = Left $ "Invalid input: " <> s
  | otherwise = do
      dir <- parseDirection ([head s])
      distance <- Read.readEither (tail s)
      return $ MovementVector dir distance

wordsBy :: (a -> Bool) -> [a] -> [[a]]
wordsBy f l = map reverse $ wordsBy' [] f l
  where
    wordsBy' :: [a] -> (a -> Bool) -> [a] -> [[a]]
    wordsBy' c f [] = [c]
    wordsBy' c f (x:xs)
      | f x = c : wordsBy' [] f xs
      | otherwise = wordsBy' (x:c) f xs

parseSteps :: String -> Either String [MovementVector]
parseSteps =
  (mapM parseMovementVector) . wordsBy (== ',')

segments :: Point -> [MovementVector] -> [Segment]
segments start [] = []
segments start (next:rest) =
  let end = addVectorToPoint next start
  in (start,end) : segments end rest

path :: String -> [Segment]
path s =
  let (Right mv) = parseSteps s
  in segments (Point 0 0) mv

pointSet :: String -> Set.Set Point
pointSet s =
  foldl' (flip Set.insert) Set.empty $ concatMap points $ path s

part1 :: String -> Int
part1 input =
  let [p1, p2] = lines input
  in head . tail . sort . map taxicabDistance . Set.toList $ Set.intersection (pointSet p1) (pointSet p2)

pointList :: [Segment] -> [Point]
pointList [] = []
pointList [p] = points p
pointList (p:ps) = (points p) <> (concat . map (tail . points) $ ps)

stepsTo :: [Segment] -> Point -> Maybe Int
stepsTo = flip elemIndex . pointList

part2' :: String -> String -> [Maybe (Int, Int)]
part2' p1 p2 =
  let
    s1 = path p1
    s2 = path p2
    intersections = tail . Set.toList $ Set.intersection (pointSet p1) (pointSet p2)
  in [(,) <$> (stepsTo s1 p) <*> (stepsTo s2 p) | p <- intersections]

part2'' :: String -> String -> Int
part2'' p1 p2 =
  minimum . (filter (/= 0)) . map (uncurry (+)) $ Maybe.catMaybes $ part2' p1 p2
...
someFunc :: IO ()
someFunc = do
  putStrLn . show $ part1 part1Input
  putStrLn . show $ part2 part1Input
